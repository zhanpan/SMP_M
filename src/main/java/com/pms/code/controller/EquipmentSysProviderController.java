...
public class EquipmentSysProviderController extends BaseController {
	...
	private Logger logger = LoggerFactory
			.getLogger(EquipmentSysProviderController.class);
	...
	public String queryEquipmentSysProviderInfo(@RequestBody String data) {
		...
		paramMap = equipmentSysProviderServiceImpl
				.queryEquipmentSysProviderInfo(paramMap);
		Page<EquipmentSysProvider> page = (Page<EquipmentSysProvider>) paramMap
				.get("page");
		if (page == null) {
			map.put("code", "201");
	...
	public String deleteEquipmentSysProviderInfo(HttpServletRequest request) {
		...
		// 调用service方法删除服务商
		boolean flag = equipmentSysProviderServiceImpl
				.deleteEquipmentSysProviderInfo(ids);
		if (flag) {
			map.put("code", "200");
			map.put("msg", "删除成功");
		} else {
	...
	public String editSysnotice(
		...
		// 根据id查询服务商
		EquipmentSysProvider equipmentSysProvider = equipmentSysProviderServiceImpl
				.findEquipmentSysProviderByID(id);
	...
	public String saveEditSysNotice(EquipmentSysProvider equipmentSysProvider) {
		boolean flag = equipmentSysProviderServiceImpl
				.updateEquipmentSysProviderInfo(equipmentSysProvider);
		if (flag) {
			map.put("code", "200");
			map.put("msg", "保存成功");
		} else {
	...
	public String insertNotice(EquipmentSysProvider equipmentSysProvider) {
		boolean flag = equipmentSysProviderServiceImpl
				.insertEquipmentSysProviderInfo(equipmentSysProvider);
		if (flag) {
			map.put("code", "200");
			map.put("msg", "添加成功");
		} else {
